...
public class AuditMobileServiceHome {
	private SessionFactory sessionFactory = getSessionFactory();
	private static final Logger log = Logger.getLogger(AuditMobileServiceHome.class);

	public void insert(Auditmobileservice auditmobileservice) {
		Session session = null;
		Transaction transaction = null;
		try {
			session = this.sessionFactory.openSession();
			transaction = session.beginTransaction();
			session.save(auditmobileservice);
			transaction.commit();
		} catch (Exception e) {
			StringWriter s;
