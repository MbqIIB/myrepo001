...
public class RegistrationHome {
	...
	public Registration getUserById(Integer regId) {
	...
	public Registration getUserByIdAndMobNo(Integer regVPAId, String mobNo) {
	...
	public Registration getUserByIdAndMobNoAndPin(Integer regVPAId, String mobNo, String pin) {
...
//			log.info("Updated [" + updated + "] rows in registration with device id same  before user register");
	...
	public boolean saveGCMTokenInRegVPA(Integer regvpaId, String mobileNo, String gcmToken) {
	...
	public List<Registration> validateServerToken(ReqJson reqJson) {
		...
		try {
			session = this.sessionFactory.openSession();
			Criteria cr = session.createCriteria(Registration.class);
			cr.add(Restrictions.eq(ConstantI.REGID, Integer.parseInt(reqJson.getRegId())));
			cr.add(Restrictions.eq(ConstantI.MOBNO, reqJson.getMobileNo()));
			results = cr.list();
		} catch (Exception e) {
			StringWriter s;
	...
	public Registration getUserByDeviceId(String regId, String deviceId) {
		...
		try {
			session = this.sessionFactory.openSession();
			Criteria cr = session.createCriteria(Registration.class);
			cr.add(Restrictions.eq("regid", Integer.parseInt(regId)));
			cr.add(Restrictions.eq("deviceid", deviceId.trim()));
			cr.add(Restrictions.eq("status", ConstantI.ACTIVE_REGVPA));
			cr.addOrder(Order.desc("createddt"));
			cr.setMaxResults(1);
	...
	public boolean updateDeviceIdIMEI(final int regId, final String deviceId, final String deviceIMEI) {
		Session session = null;
		Transaction tx = null;
		try {
			...
			updated = session.createSQLQuery(updateDeviceIdNIMEI).setString(ConstantI.DEVICE_ID, deviceId.trim())
					...
					.setInteger(ConstantI.REGID, regId).executeUpdate();
			tx.commit();
			return updated >= 1;
		} catch (Exception e) {
			StringWriter s;
