...
public class SilentsmsHomeService {
	...
	public RespJson getMobileNoService(ReqJson reqJson) {
		...
		try {
			...
			/*AesEncryption aESEncryptionUtility		= new AesEncryption();
			String encKey=ConstantI.KEY_DECRY_HEADER;
			log.info("Encrypted deviceId for getMobile=" + reqJson.getDeviceId());
			String deviceid = aESEncryptionUtility.decrypt(reqJson.getDeviceId(), encKey);
			log.info("Decrypted deviceId for getMobile=" + deviceid);
