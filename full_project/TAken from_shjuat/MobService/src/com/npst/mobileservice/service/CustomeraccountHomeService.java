...
public class CustomeraccountHomeService {
	...
	public RespJson deleteAll(ReqJson reqJson) {
		...
		try {
			...
			List<Customeraccount> listCustAcc = customeraccountHome.getListAccountByRegVPAId(Long.parseLong(reqJson.getRegId()));
	...
	public RespJson getAddedAccount(ReqJson reqJson) {
		...
		try {
			...
			List<Customeraccount> accountList = customeraccountHome.getListAccountByRegVPAId(Long.parseLong(reqJson.getRegId()));
	...
	public List<Customeraccount> getVpa(ReqJson reqJson) {
		...
		try {
			...
			accountList = customeraccountHome.getAllVpas(Long.parseLong(reqJson.getRegId()));
		} catch (Exception e) {
			respJson.setMsgId(ConstantI.MSGID_FAIL);
			respJson.setMsg(ErrorCode.AcqErrorCode.MSGID_EXCEPTIONMSG.getCode());
			StringWriter s;
	...
	public RespJson insertBankAcc(ReqJson reqJson) {
		...
		try {
			...
			List<Customeraccount> resultCustomeraccount = customeraccountHome
					.getListAccountByRegVPAId(Long.parseLong(reqJson.getRegId()));
			...
			customerAccount.setRegid(Long.parseLong(reqJson.getRegId()));
	...
	public RespJson setDefault(ReqJson reqJson) {
		...
		try {
			...
			List<Customeraccount> listaccount = customeraccountHome
					.getActiveListAccountByVPAAndRegVPAId(reqJson.getVirtualId(), Long.parseLong(reqJson.getRegId()));
	...
	public RespJson getAllVPAByAccount(ReqJson reqJson) {
		...
		try {
			...
			List<String> customerAccList = customeraccountHome.getActiveInactiveVPAForAccount(reqJson.getAccRefNumber(),
					Long.parseLong(reqJson.getRegId()));
	...
	public RespJson getNonPrimaryAccVPA(ReqJson reqJson) {
		...
		try {
			...
			custAccList = customeraccountHome.getNonPrimaryAccVPA(Long.parseLong(reqJson.getRegId()));
	...
	public RespJson accountStatus(ReqJson reqJson) {
		...
		try {
			...
			if (customeraccountHome.updateAccount(Long.parseLong(reqJson.getRegId()), reqJson.getVirtualId(), reqJson.getAccRefNumber(),
					reqJson.getStatus())) {
	...
	public RespJson vpaStatus(ReqJson reqJson) {
		...
		try {
			...
			if (customeraccountHome.updateVpa(Long.parseLong(reqJson.getRegId()), reqJson.getVirtualId(), reqJson.getStatus())) {
				respJson.setMsgId(ConstantI.MSGID_SUCCESS);
	...
	public RespJson getDefInfo(ReqJson reqJson) {
		...
		try {
			...
			Registration registration = registrationHome.getUserById(Long.parseLong(reqJson.getRegId()));
			Customeraccount custAcc = customeraccountHome.getDefAcc(Long.parseLong(reqJson.getRegId()),
				...
				if (newCustList != null && newCustList.size() > 0) {
					...
					if (cc == null) {
						...
						if (checkPrimaryAcc(newCustList)) {
							...
							if (cc != null) {
								f = customeraccountHome.updateNonPrimaryByCaidNotIn(Long.parseLong(reqJson.getRegId()), cc, ConstantI.NORMAL_ACCOUNT);
							}
						} else {
							f = customeraccountHome.saveListOfCustAccount(newCustList);
						}
